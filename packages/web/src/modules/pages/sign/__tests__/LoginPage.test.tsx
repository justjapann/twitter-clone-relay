...
it('should display an error and disable the button when type in username input', async () => {
  ...
  expect(screen.getByRole('button')).toBeDisabled()
  expect(screen.getByTestId('error-message-username')).toHaveTextContent('Username is required')
...
it('should display an general error on the form if return error from the mutation', async () => {
  ...
  userEvent.type(screen.getByPlaceholderText('Password'), 'thisisanawkwardpassword')
  ...
  expect(screen.getByTestId('form-error-text')).toHaveTextContent('Some error message from backend')
})
